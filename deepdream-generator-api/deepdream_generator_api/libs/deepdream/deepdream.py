...
from deepdream_generator_api.libs.deepdream import inception5h
...
def tiled_gradient(session, gradient, image, tile_size=400):
...
def optimize_image(
    ...
    for i in range(num_iterations):
        ...
        # TODO could this be a good place for updating progress
        print(". ", end="")
    return img
...
def make_deepdream_image(image, params: DeepdreamParams, progress_callback):
    session = tf.InteractiveSession(graph=model.graph)
    image_array = np.float32(image)
    layer_tensor = model.layer_tensors[params.layer_tensor_index]
    result_image = recursive_optimize(
        image=image_array,
        session=session,
        progress_callback=progress_callback,
        layer_tensor=layer_tensor,
    ...
    # Ensure the pixel-values are between 0 and 255.
    result_image = np.clip(result_image, 0.0, 255.0)
